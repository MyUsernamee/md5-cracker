...
__device__ inline void md5Hash(unsigned char* data, uint32_t length, uint32_t *a1, uint32_t *b1, uint32_t *c1, uint32_t *d1){
	...
	// pretend like this was already here :)
	for(i = 0; i < PREFIX_LENGTH; i++){
		vals[i / 4] |= PREFIX[i] << ((i % 4) * 8);
	}

	for(i = PREFIX_LENGTH; i < length; i++){
		vals[i / 4] |= data[i - PREFIX_LENGTH] << ((i % 4) * 8);
	}
	...
	// Weird java edit (nameUUIDFromBytes)
	b &= 0xFF0FFFFF; /* clear version        */
	b |= 0x00300000; /* set to version 3     */
	c &= 0xFFFFFF3F; /* clear variant        */
	c |= 0x00000080; /* set to IETF variant  */
