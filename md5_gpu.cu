...
char g_word[CONST_WORD_LENGTH_MAX];
char g_charset[CONST_CHARSET_LENGTH];
char g_cracked[CONST_WORD_LENGTH_MAX];

__device__ char g_deviceCharset[CONST_CHARSET_LENGTH];
__device__ char g_deviceCracked[CONST_WORD_LENGTH_MAX];

__device__ __host__ bool next(uint8_t* length, char* word, uint32_t increment){
	uint32_t idx = 0;
	uint32_t add = 0;

	while(increment > 0 && idx < CONST_WORD_LENGTH_MAX){
...
__global__ void md5Crack(uint8_t wordLength, char* charsetWord, uint32_t hash01, uint32_t hash02, uint32_t hash03, uint32_t hash04){
	...
	/* Thread variables */
	char threadCharsetWord[CONST_WORD_LENGTH_MAX];
	char threadTextWord[CONST_WORD_LENGTH_MAX];
	uint8_t threadWordLength;
	uint32_t threadHash01, threadHash02, threadHash03, threadHash04;

	/* Copy everything to local memory */
	memcpy(threadCharsetWord, charsetWord, CONST_WORD_LENGTH_MAX);
...
int main(int argc, char* argv[]){
	...
	/* Fill memory */
	memset(g_word, 0, CONST_WORD_LENGTH_MAX);
	memset(g_cracked, 0, CONST_WORD_LENGTH_MAX);
	...
	for(int device = 0; device < devices; device++){
		...
		ERROR_CHECK(cudaMemcpyToSymbol(g_deviceCracked, g_cracked, sizeof(uint8_t) * CONST_WORD_LENGTH_MAX, 0, cudaMemcpyHostToDevice));
		
		/* Allocate on each device */
		ERROR_CHECK(cudaMalloc((void**)&words[device], sizeof(uint8_t) * CONST_WORD_LENGTH_MAX));
	}
	...
	while(true){
		...
		for(int device = 0; device < devices; device++){
			...
			/* Copy current data */
			ERROR_CHECK(cudaMemcpy(words[device], g_word, sizeof(uint8_t) * CONST_WORD_LENGTH_MAX, cudaMemcpyHostToDevice)); 
		...
		/* Display progress */
		char word[CONST_WORD_LENGTH_MAX];
		...
		for(int device = 0; device < devices; device++){
			...
			/* Copy result */
			ERROR_CHECK(cudaMemcpyFromSymbol(g_cracked, g_deviceCracked, sizeof(uint8_t) * CONST_WORD_LENGTH_MAX, 0, cudaMemcpyDeviceToHost)); 
